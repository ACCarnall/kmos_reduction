...
class pointing(object):
    ...
    def plot_images(self, crop=0, centroids=None, collapsed_cube=False,
        ...
        for i in range(24):
            ...
            if pd.isnull(self.cat.loc[i+1, "object"]):
                continue

            axes.append(plt.subplot(gs[int(np.round(i/float(rowno) - i%rowno/float(rowno))), i%rowno]))
            plt.setp(axes[-1].get_xticklabels(), visible=False)
            plt.setp(axes[-1].get_yticklabels(), visible=False)

            if centroids is not None:
                cent = centroids.loc[self.cat.loc[i+1, "object"], ["x", "y"]].values

            else:
                cent = None

            self.cat.loc[i+1, "cube"].add_image(axes[-1], crop=crop,
                                                centroid=cent,
                                                collapsed_cube=collapsed_cube)

            if self.cat.loc[i+1, "object"].endswith("SELECT"):
                axes[-1].set_title(self.cat.loc[i+1, "object"][:-6], fontsize=9)

            else:
                axes[-1].set_title(self.cat.loc[i+1, "object"], fontsize=9)
